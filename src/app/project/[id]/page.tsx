...
import { getProjectById, getDocumentReadiness } from "@/lib/data";
...
export default function ProjectDetailPage({
  ...
  const missingDocs = project.documents.filter(
  ...
  const activityLog = [
    ...
    ...project.documents
      ...
      .map((d) => ({
        date: d.uploadedAt!,
        text: `${d.docLabel} uploaded`,
      })),
    {
      date: project.updatedAt,
      text: `Status changed to ${STATUS_LABELS[project.status]}`,
    },
    ...
    <div className="py-8 px-4 sm:px-6">
      <div className="mx-auto max-w-4xl">
        ...
        {missingDocs.length > 0 && project.status !== "complete" && (
          <Alert className="bg-amber-50 border-amber-200 mb-6">
            <AlertDescription className="text-amber-800">
              <strong>Missing {missingDocs.length} required document{missingDocs.length !== 1 ? "s" : ""}:</strong>{" "}
        ...
        {rejectedDocs.length > 0 && (
          <Alert className="bg-red-50 border-red-200 mb-6">
            <AlertDescription className="text-red-800">
              <strong>{rejectedDocs.length} document{rejectedDocs.length !== 1 ? "s" : ""} rejected:</strong>{" "}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            {/* Next Action */}
            {missingDocs.length > 0 && project.status !== "complete" && project.status !== "denied" && (
              <Card className="border-[#2563eb]">
                <CardContent className="pt-6">
                  <p className="font-semibold text-gray-900 mb-2">Next Step</p>
                  <p className="text-sm text-gray-600">
                    Upload remaining {missingDocs.length} document{missingDocs.length !== 1 ? "s" : ""} to submit
                    your application.
                  </p>
                </CardContent>
              </Card>
